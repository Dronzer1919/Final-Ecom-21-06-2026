...
@Component({
  ...
  styleUrls: ['./frequently-bought.component.scss']
})
export class FrequentlyBoughtComponent implements OnInit {
  products: Product[] = [];
  loading = false;
  ...
  loadProducts(): void {
    ...
    this.productService.getAllProducts(this.currentPage, this.pageSize)
      ...
      .subscribe({
        next: (response) => {
          // Randomize products to simulate frequently bought together
          this.products = [...(response.data.products ?? [])].sort(() => 0.5 - Math.random());
