...
@Component({
  ...
  templateUrl: './frequently-bought.page.html',
  styleUrls: ['./frequently-bought.page.scss']
})
export class FrequentlyBoughtPage implements OnInit, AfterViewChecked, OnDestroy {
    ...
    this.productService.getAllProducts(this.currentPage, this.pageSize)
      ...
      .subscribe({
        next: (response) => {
          ...
          const arr: Product[] = Array.isArray(data) ? data : (data || []);
          this.products = [...arr].sort(() => 0.5 - Math.random());
  ...
  loadMoreProducts(): void {
    ...
    this.productService.getAllProducts(this.currentPage, this.pageSize)
      ...
      .subscribe({
        next: (response) => {
          const data = response.data;
          const newProducts: Product[] = Array.isArray(data) ? data : [];
  ...
  getProductId(product: Product): string { return product._id || product.id || ''; }
  getProductImage(product: Product): string { return product.image || (product.images?.[0]) || 'assets/images/product-placeholder.svg'; }
  ...
  getDiscountPercentage(product: Product): number {
    ...
    if (product.discountType === 'fixed' && product.discountValue) return Math.round((product.discountValue / product.price) * 100);
    if (product.discountPrice && product.price) return Math.round(((product.price - product.discountPrice) / product.price) * 100);
    return 0;
  }
